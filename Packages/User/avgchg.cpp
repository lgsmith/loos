/*
  ...
  Usage:
    avgchg [options] model traj >average.pdb
...
string fullHelpMessage(void) {
  string msg =
    "\n"
    "SYNOPSIS\n"
    "Compute an average structure with charges from a trajectory\n"
    "\n"
    "DESCRIPTION\n"
    "\tavgchg writes out a PDB for the average structure with charges in the b-factor column from \n"
    "a trajectory.  If a selection is given (--selection), then the trajectory is first iteratively\n"
    "aligned to an optimal average structure (see aligner).  The '--average' option takes an\n"
    "optional selection that defines what atoms are averaged and written out, otherwise all\n"
    "non-hydrogen and non-solvent atoms are used. Note that solvent is selected by a segid\n"
    "of either 'BULK' or 'SOLVENT'. If your system uses a different identifier, you will want\n"
    "to explicitly give a selection for the --average option. You need to use a model that\n"
    "includes charges for this tool. The charge units will match those of the model.\n"
    "\n"
    "EXAMPLES\n"
    "\n"
